  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (patientData) {
        ...
        setMessage('Paciente não encontrado.')
      }
      setLoading(false)
    }
    loadData()
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8 border-b pb-4">
        ...
        {patient && (
          <p className="text-gray-600 mt-2">Paciente: <strong className="text-gray-800">{patient.full_name}</strong></p>
        )}
      </div>

      {message && <div className={`mb-6 p-4 rounded ${message.includes('Erro') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>{message}</div>}
      ...
      <div className="space-y-8 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="space-y-3">
              {area.items.map((item) => (
                <label key={item.id} className="flex items-start gap-3 p-3 rounded hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-100 transition-all">
                  <input type="checkbox" className="mt-1 h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked={responses[item.id] || false} onChange={() => toggleResponse(item.id)} />
        ...
        <button onClick={handleSave} disabled={loading || !patient} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 transition-colors disabled:opacity-50">
